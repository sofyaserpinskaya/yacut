...
ORIGINAL_LENGTH_ERROR = (
    'Недопустимо длинная ссылка. Ваша ссылка - {length} символов. '
    'Максимальная длина - {acceptable}.'
)
...
class URL_map(db.Model):
    ...
    def get_unique_short_id():
        for _ in range(ITERATION_NUMBER):
            ...
            if URL_map.get_url_map(short_id) is None:
                return short_id
    ...
    def create(original, short_id=None, validate=False):
        if validate:
            ...
            if original_length > ORIGINAL_LENGTH:
                raise ValueError(ORIGINAL_LENGTH_ERROR.format(
                    length=original_length, acceptable=ORIGINAL_LENGTH
                ))
            if short_id:
